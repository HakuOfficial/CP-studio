...
## v0.3.8 Windows runner notes

- Memory monitoring no longer starts `tasklist.exe` repeatedly; Windows uses native process/job memory APIs.
- Contest programs are launched with no console window, preventing the flashing seen in packaged builds.
- `Settings -> Editor & Shortcuts -> Operator ligatures` can be disabled for Code::Blocks-style literal operators such as `!=` and `->`.
- Glass materials are reset with Tauri `clearEffects()` before applying a new effect.
