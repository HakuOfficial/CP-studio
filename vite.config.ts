...
export default defineConfig(({ mode }) => {
  ...
  return {
    ...
    server: {
      ...
      strictPort: true,
      // Vite only owns the React frontend. Rust/Tauri is watched by the
      // Tauri CLI itself. Watching src-tauri (especially target/) on Windows
      // can crash Node with EBUSY when Cargo has .exe/.pdb files locked.
      watch: {
        ignored: [
          '**/.figma/**',
          '**/src-tauri/**',
